...
0x00, 0x00, 0x50, 0xE3, 0x08, 0x00, 0x9F, 0x05,
0x00, 0x00, 0xA0, 0x13, 0x00, 0x00, 0x90, 0x05
};

static const QVector<quint8> wcstoulAddrPattern = {
0x25, 0x88, 0xA4, 0x1C,
0x00, 0x2D, 0x04, 0xD0
};
...
static const QVector<quint8> QRAlwaysUnlockedPattern = { //-0xC
0x01, 0x50, 0x80, 0xE0, 0x06, 0x00, 0xD5, 0xE5,
...
static const QVector<quint8> CmdKeyboardHookPattern = { //-0x10
0x4C, 0x00, 0x94, 0xE5, 0x4C, 0x50, 0x84, 0xE2,
0x08, 0x10, 0x90, 0xE5, 0x05, 0x00, 0xA0, 0xE1
};

static const QVector<quint8> CmdKeyboard_Function = {
    /* Loader function; Allows us to do just BL in the game's function instead of LDR and BL; saves one instruction place */
    0xFE, 0x5F, 0x2D, 0xE9, //STMFD SP!, {R1-R12,LR}
    0x50, 0x00, 0x94, 0xE5, //LDR R0, [R4,#0x50]: Get wchar_t pointer for keyboard text
    0x04, 0x10, 0x8F, 0xE2, //ADD R1, PC, #4 //+4 instead of 12 due to how PC works teehee (PC = cur instruction + 8)
    0x31, 0xFF, 0x2F, 0xE1, //BLX R1 //Branch to the code below
    0xFE, 0x9F, 0xBD, 0xE8, //LDMFD SP!, {R1-R12,PC}
    ...
    0x35, 0xff, 0x2f, 0xe1, 0x04, 0x00, 0xa0, 0xe1, 0xd8, 0xff, 0xff, 0xea,
    0x35, 0x08, 0x10, 0x00, //wcslen
    0x00, 0x00, 0x00, 0x00, //rodata offset here
    0x65, 0x18, 0x10, 0x00 //wcsncmp
};

static const QVector<quint8> CmdKeyboard_FunctionRoData = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, //func_ptrs
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,

    0x05, 0x07, 0x07, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, //func_cmd_size
    0x6c, 0x00, 0x00, 0x00, 0x76, 0x00, 0x00, 0x00, 0x82, 0x00, 0x00, 0x00, 0x92, 0x00, 0x00, 0x00, //func_cmds
    0xa4, 0x00, 0x00, 0x00, 0xac, 0x00, 0x00, 0x00, 0xb4, 0x00, 0x00, 0x00, 0xbc, 0x00, 0x00, 0x00,
    0xc4, 0x00, 0x00, 0x00, 0xcc, 0x00, 0x00, 0x00, 0xd4, 0x00, 0x00, 0x00, 0xdc, 0x00, 0x00, 0x00,

    //Let's make tons of space for our strings
...
static const QVector<quint8> CmdKeyboard_Text2Item_Function = {
    0x0D, 0xC0, 0xA0, 0xE1, 0x00, 0x10, 0xA0, 0xE3, 0x00, 0xD8, 0x2D, 0xE9, 0x10, 0x20, 0xA0, 0xE3,
    0x58, 0x30, 0x9F, 0xE5, 0x04, 0xB0, 0x4C, 0xE2, 0x33, 0xFF, 0x2F, 0xE1, 0x00, 0x30, 0xA0, 0xE1,
    0x02, 0x29, 0xC0, 0xE3, 0x48, 0x00, 0x9F, 0xE5, 0x30, 0xFF, 0x2F, 0xE1, 0x00, 0x00, 0x52, 0xE3,
    0x00, 0x00, 0x50, 0x13, 0x00, 0xA8, 0x9D, 0x08, 0x38, 0x10, 0x9F, 0xE5, 0x83, 0x38, 0xA0, 0xE1,
    0xA3, 0x38, 0xA0, 0xE1, 0x01, 0x00, 0x53, 0xE1, 0xF9, 0xC0, 0x43, 0xE2, 0x00, 0x30, 0xA0, 0x93,
    0x01, 0x30, 0xA0, 0x83, 0x20, 0x10, 0x9F, 0xE5, 0x01, 0x00, 0x5C, 0xE1, 0x01, 0x30, 0x83, 0x93,
    0x00, 0x00, 0x53, 0xE3, 0x6B, 0x0C, 0x80, 0x02, 0xD0, 0x20, 0x80, 0x05, 0x00, 0xA8, 0x9D, 0xE8,
    0xC9, 0x13, 0x10, 0x00, //wcstoul
    0xC8, 0xBA, 0x2F, 0x00, //Get_Base_Player_Address
    0xFE, 0x7F, 0x00, 0x00, 0x07, 0x1F, 0x00, 0x00
};

static const QVector<quint8> CmdKeyboard_ChangeTime_Function = {
    0x0D, 0xC0, 0xA0, 0xE1, 0x00, 0xD8, 0x2D, 0xE9, 0x04, 0xB0, 0x4C, 0xE2, 0x00, 0xA8, 0x9D, 0xE8
};

static const QVector<quint8> CmdKeyboard_DupeAll_Function = {
    0x0D, 0xC0, 0xA0, 0xE1, 0x34, 0x00, 0x9F, 0xE5, 0x00, 0xD8, 0x2D, 0xE9, 0x04, 0xB0, 0x4C, 0xE2,
    0x30, 0xFF, 0x2F, 0xE1, 0x00, 0x00, 0x50, 0xE3, 0x00, 0xA8, 0x9D, 0x08, 0x6B, 0x2C, 0x80, 0xE2,
    0xD0, 0x30, 0x82, 0xE2, 0x1B, 0x0B, 0x80, 0xE2, 0xD0, 0x20, 0x92, 0xE5, 0x10, 0x00, 0x80, 0xE2,
    0x04, 0x20, 0x83, 0xE4, 0x03, 0x00, 0x50, 0xE1, 0xFC, 0xFF, 0xFF, 0x1A, 0x00, 0xA8, 0x9D, 0xE8,
    0xC8, 0xBA, 0x2F, 0x00 //Get_Base_Player_Address
};

static const QVector<quint8> CmdKeyboard_ClearInv_Function = {
    0x0d, 0xc0, 0xa0, 0xe1, 0x34, 0x00, 0x9f, 0xe5, 0x00, 0xd8, 0x2d, 0xe9, 0x04, 0xb0, 0x4c, 0xe2,
...
Patch UnusedCode;
Patch UnusedRoData;
Patch GetBasePlayerAddr; //Pattern: 00 00 50 E3 08 00 9F 05 00 00 A0 13 00 00 90 05 - 0x8
Patch wcstoulAddr; //Thumb; Pattern: 25 88 A4 1C 00 2D 04 D0 - 0x14
...
Patch CmdKeyboardFunction;
Patch CmdKeyboardRoData;

Patch CmdKeyboard_Text2Item;
Patch CmdKeyboard_ChangeTime;
Patch CmdKeyboard_DupeAll;
...
Patch::Patch(const QVector<quint8> Pattern, QVector<quint8> Values, quint32 OffsetFromPattern) {
    Game* game = MainWindow::GetGameInstance();
    game->codebin->m_file->seek(0);
    QByteArray data = game->codebin->m_file->readAll();
...
bool Patch::Apply(File* code, quint8 OR) {
    ...
    else if (!m_Valuesu8.empty()) {
        res = false;
        QByteArray array(reinterpret_cast<const char*>(this->m_Valuesu8.constData()), this->m_Valuesu8.size());
        res |= !code->Write(m_Offset - 0x100000, array);
    }
...
void Patch::Init(void) {
    ...
    GetBasePlayerAddr= Patch(GetBasePlayerAddrPattern, QVector<PatchValues>({{0,0}}), static_cast<quint32>(-8));
    wcstoulAddr      = Patch(wcstoulAddrPattern, QVector<PatchValues>({{0,0}}), static_cast<quint32>(-0x14));
    wcstoulAddr.m_Offset+=1; //Show it's Thumb offset when wanting to jump to it
    qDebug() << "End General Utils";

    /* Command Keyboard Utils */
    CmdKeyboardFunction=Patch(UnusedCode_Pattern, MakeARMPatchValuesFromFunctionBytes(CmdKeyboard_Function), 0);
    CmdKeyboardRoData = Patch(UnusedRoData_Pattern, CmdKeyboard_FunctionRoData, 0);
    CmdKeyboard_Text2Item = Patch(UnusedCode_Pattern, MakeARMPatchValuesFromFunctionBytes(CmdKeyboard_Text2Item_Function),0);
    CmdKeyboard_ChangeTime = Patch(UnusedCode_Pattern, MakeARMPatchValuesFromFunctionBytes(CmdKeyboard_ChangeTime_Function),0);
    CmdKeyboard_DupeAll = Patch(UnusedCode_Pattern, MakeARMPatchValuesFromFunctionBytes(CmdKeyboard_DupeAll_Function),0);
    CmdKeyboard_ClearInv = Patch(UnusedCode_Pattern, MakeARMPatchValuesFromFunctionBytes(CmdKeyboard_ClearInv_Function),0);
    qDebug() << "End Cmd Keyboard Utils";
